...
export const ChartLib = {
    ...
    clearModelCharts: () => {
        for(const key in _charts){
            if(["model-jitter", "model-latency", "model-loss"].includes(key)) delete _charts[key]
        }
    },
    colors: {
        BACK: "#353B3E",
    ...
    palettes: {
        ...
        model: ["#D77420", "#27D507", null, null]
    },
    axes: {
        font: { family: "Open Sans", size: 12, color: "#666666" }
    },
    ...
    bar: (id, data, options) => {
        ...
        if (_charts[id]) {
            ...
            _charts[id].options.scales.yAxis.ticks.callback = o => {
                const f = fmt(options.yfmt || "n");
                return f(o);
            };
            const tcb = _charts[id].options.plugins.tooltip.callbacks;
                ...
                plugins: [
                    {
                        afterDraw: function (chart) {
                            ...
                            const fontSize = chart.options.font.size;
